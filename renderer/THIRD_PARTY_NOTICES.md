...
links the following third-party components. Provenance for source revisions
is in `UPSTREAM.md`; the distribution strategy is in `LICENSING.md`.

## Telegram Desktop

- Copyright the Telegram Desktop contributors.
- License: GPL-3.0-only with the OpenSSL exception
  (`upstream/tdesktop/LEGAL`, `upstream/tdesktop/LICENSE`).
- Used as: pinned submodule + patch stack; compiled into the native harness
  and the WASM artifact.

## desktop-app libraries (lib_ui, lib_base, lib_rpl, lib_crl, lib_style, codegen)

- Copyright the desktop-app contributors.
- License: GPL-3.0-only with the OpenSSL exception.
- Used as: submodules of the pinned TDesktop checkout, linked into the
  renderer.
...
- Native harness: distro Qt 6 packages (Ubuntu). WASM artifact: Qt for
  WebAssembly built per `scripts/build-wasm.sh`; the exact Qt revision used
  for any released artifact must be recorded in the release manifest.
...
- Any release that embeds font files must list the exact files and their
  license texts here before shipping. No release in this branch ships fonts
  yet.
